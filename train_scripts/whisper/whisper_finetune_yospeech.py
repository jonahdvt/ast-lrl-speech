...
max_yo_samples = 2077              # or whatever you prefer
fleurs_lang = "yo_ng"              # FLEURS Yoruba
max_fleurs_samples = None          # use full FLEURS split

def load_ys_stream(split="train", max_samples=None):
    ds = load_dataset(
        "yo-speech/yo-asr-speech",
        split=split,
        streaming=True,
    )
    if max_samples:
    ...
    def normalize_ys(batch: Dict[str, Any]) -> Dict[str, Any]:
        arr = batch["audio"]["array"]
        sr = batch["audio"]["sampling_rate"]
        return {
            "audio_array": arr,
            "sampling_rate": sr,
            "sentence": batch["transcript"],
        }
    return ds.map(
        normalize_ys,
...
def load_fleurs_stream(lang="yo_ng", split="train", max_samples=None):
    ds = load_dataset(
        "google/fleurs",
        name=lang,
        split=split,
...
ys_ds  = load_ys_stream(max_samples=max_yo_samples)
fle_ds = load_fleurs_stream(fleurs_lang, max_samples=max_fleurs_samples)
train_stream = interleave_datasets([ys_ds, fle_ds])
...
training_args = Seq2SeqTrainingArguments(
    output_dir="./whisper-large-yo-ys-2.5h",
...
trainer.push_to_hub(
    ...
    dataset="YorubaSpeech",
    language="yo",
    model_name="Whisper Large — Yoruba 2.5h)",
...
processor.push_to_hub("jonahdvt/whisper-large-yo-ys-2.5h")
